...
!            patch%itype(pi:pf) = ispval
   ...
   subroutine restart( this, bounds_proc, ncid, flag, waterdiagnosticbulk_inst, &
     ...
     use FatesInterfaceTypesMod, only:  fates_maxElementsPerSite
      ...
      integer                 :: c   ! HLM column index
      integer                 :: s   ! Fates site index
!comment out for HH-FATES
      !scs jks      !integer                 :: g   ! grid-cell index
      !scs jks
      intger                  :: s0  ! global site offset for the current clump
     ...
     call t_startf('fates_restart')
      ...
      if(.not.initialized) then
         ...
         do nc = 1,nclumps
            ...
            ! site on this clump.
            s0 = floor(real(bounds_clupm%begCohort,r8)/real(fates_maxElementsPerSite,r8)
            !scs jks END
            ...
            !scs jks !g = col%gridcell(c)
            !scs jks !this%fates_restart%restart_map(nc)%cohort1_index(s) = (g-1)*fates_maxElementsPerSite + 1
               ! scs jks modify for HH-FATES restarts
